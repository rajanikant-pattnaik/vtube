...
const Page = () => {
  ...
  const deleteALL=async()=>{
     if(id!==''){
      try {
        const res=await axios.post('/api/wl/deleteAll',{userid:id});
        console.log(res);
  ...
  useEffect(() => {
    const getHistory=async()=>{
      try {
         if(id!==''){
          const res=await axios.post('/api/wl/getAllWl',{userId:id});
          console.log(res.data.allwl);
          setdata(res.data.allwl)
         }
      } catch (error:any) {
